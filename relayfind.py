...
parser.add_argument('-t', '--timeout', default=3, type=int, metavar='SEC', help='seconds to wait for relay board to reply (default: %(default)s)')
...
try:
    while True:
        ...
        found.append({"mac": mac, "ip": ip, "name": name, "version": version})
        if not args.json:
            print('Found a relay:')
    ...
    if found:
        if args.json: print(json.dumps({"error": 0, "devices": found}))
        exit(0)
